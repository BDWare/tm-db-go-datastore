...
// https://github.com/tendermint/tm-db/blob/3157a928986298875ca48e6d5f77132a32dfb1f0/common_test.go
package db
...
func newTempDB(dirPattern string) (db db.DB, dbDir string, err error) {
	dbDir, err = ioutil.TempDir("", dirPattern)
	if err != nil {
		return
	}
	ds, err := leveldb.NewDatastore(dbDir, key.KeyTypeBytes, &leveldb.Options{SyncWrites: false})
	if err != nil {
		return
	}
	return New(ds), dbDir, nil
}

func benchmarkRangeScans(b *testing.B, db db.DB, dbSize int64) {
	b.StopTimer()
